...
def validate_maintenance_window(window):
    ...
    hour = r'[01]?[0-9]|2[0-3]'
    minute = r'[0-5][0-9]'
    r = f"(?P<start_day>{day_re}):(?P<start_hour>{hour}):(?P<start_minute>{minute})-(?P<end_day>{day_re}):(?P<end_hour>{hour}):(?P<end_minute>{minute})"
    range_regex = re.compile(r)
    m = range_regex.match(window)
    if not m:
    ...
    if m['start_day'] not in days or m['end_day'] not in days:
        raise ValueError(
            f'DBInstance PreferredMaintenanceWindow day part of ranges must be one of: {", ".join(days)}'
        )
    start_ts = (
        days.index(m['start_day']) * 24 * 60
        + int(m['start_hour']) * 60
        + int(m['start_minute'])
    )
    end_ts = (
        days.index(m['end_day']) * 24 * 60
        + int(m['end_hour']) * 60
        + int(m['end_minute'])
    )
    if abs(end_ts - start_ts) < 30:
